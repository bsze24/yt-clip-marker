...
### D-035 — the export merges only entries that would print the same words  (Accepted 2026-08-19)
`mergeNearby` now requires `it.label === cluster.clip.label` alongside the 2-second window.
Entries that are close in time but carry different wording each get their own line.

[[D-033]] says every marker Brian made reaches the exported description. Until now the code did
not enforce that: the merge compared times and ranks and never read the text, so two entries
inside the window collapsed to one and the loser's wording vanished with no warning.
...
**The loss it prevents has never occurred, and that is not the reason to ship it.** Across
video 1's 64 skill markers and 21 added markers there are zero skill+skill, zero added+added and
zero skill+added pairs within 2 seconds. In the 42 minutes carrying no extracted markers, 44
markers sit there and the tightest pair is 7 seconds apart. Every one of the 15 collisions on
that video was a marker meeting an **extracted marker**, which rewrites both labels to the same
text first, so the merge only ever collapsed duplicates.

The reason to ship it is that the alternative was a check to run after every annotated video,
which is a recurring cost forever against a one-time ten-minute change. `TD-16` is closed.
